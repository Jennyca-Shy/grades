...
const allHolidaysRaw = ref([]);
async function getAllHolidays() {
  const response = await fetch('http://localhost:3000/holiday');
  const data = await response.json();
  allHolidaysRaw.value = data;

  console.log('Rawwww: ', allHolidaysRaw.value);
}

const displayedHolidays = computed(() => {
  const nextMonthLastDayDate = new Date(
    today.value.getFullYear(),
    today.value.getMonth() + 1,
    nextMonthDays.value,
  );
  const lastMonthStartingDayDate = new Date(
    today.value.getFullYear(),
    today.value.getMonth(),
    lastMonthStartingDay.value,
  );
  console.log('nextMonthLastDayDate: ', nextMonthLastDayDate);
  console.log('lastMonthStartingDayDate: ', lastMonthStartingDayDate);
  allHolidaysRaw.filter((hol) => {
    return hol.startDate > lastMonthStartingDayDate && hol.endDate < nextMonthLastDayDate;
  });
});

const holidaysLastMonth = computed(() => {
  let holiday = [];
  let currDate = new Date(today.value.getFullYear(), today.value.getMonth() - 1, 0);
  let lastMonthDays = new Date(today.value.getFullYear(), today.value.getMonth() - 1, 0).getDate();
  for (let i = 0; i < lastMonthDays; i++) {
    const daysHoliday = allHolidaysRaw.value.filter((hol) => {
      return hol.startDate >= currDate && hol.endDate <= currDate;
    });
    holiday.push(daysHoliday);
...
<template>
  ...
  >
    ...
    >
      <div v-for="day in lastMonthList" class="flex justify-center flex-col text-gray-400">
        {{ day }}
        <Appointment
          v-for="hol in holidaysLastMonth[day - 1]"
          :title="hol.name"
          color="#4d6c9e"
          module="single"
        />
